...
| `api.theme.setVariables(vars)` | 写入 `--*` CSS 变量（主题/布局/背景） |
| `api.theme.addStyle(css)` | 注入任意 CSS（按插件隔离） |
| `api.theme.setBodyAttribute(name, value)` | 设置 `body` 属性（如 `data-theme`） |
| `api.theme.setBodyClass(name, enabled)` | 切换 `body` 类（如 `compact`） |
...
- **图床设置**（`plugins/image-host.js`）：新增“粘贴图片默认保存位置”设置，默认 **云端图床**（链接即 OSS/自定义域名 URL），可改为“本地 `images/`”。云端默认 **阿里云 OSS**（Endpoint 支持简写：`oss-cn-beijing`、`oss-cn-beijing.aliyuncs.com`、`bucket.oss-cn-beijing` 均会自动补全；自定义网关域名原样使用）：可选择 **PUT 直传**（V1 签名）或 **表单直传（POST + policy 签名，兼容性更好）**，支持自定义域名/CDN；可切换“自定义 multipart 接口”（Bearer 令牌 + 字段 `file`）。上传实现见 `web_wasm/image-host.js`；云端配置不完整或上传失败会自动回退本地并给出可操作提示（如 405 提示检查 Endpoint 是否为 OSS 域名、改用表单直传、CORS 是否允许 PUT/POST）。插件命令“测试图床上传”可上传一张 1×1 PNG 验证配置。
...
- **主题与布局**（`plugins/theme.js`）：明亮/暗色/暖色主题、强调色、编辑字号、预览背景、侧栏宽度、紧凑布局；演示 `api.theme` 与 `onSettingsChanged` 即时生效。
...
- **图床与资源**：设置上传端点与令牌后，粘贴图片自动上传并插入 `#image("https://…")`；未开启或上传失败时保存到本地 `images/` 并插入本地引用。

...
| **P2 浏览器接入** | 编译 `typstbit_core.wasm`（wasm-gc，导出 `core_init/update/exports…`）；DOM 工作台通过 JS 宿主调用核心决定 UI 结构与状态，CodeMoonBit 仍负责文本；插件注册进 MoonBit 注册表 | 9 套 e2e 全绿（先用 `?core=mbt` 开关，再切默认）；命令/大纲/筛选与 JS 版逐项一致 |

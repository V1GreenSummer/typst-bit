...
体验修订（2026-09-21）：修复连续中文 IME 提交覆盖上一个字（合成提交的光标映射）；删除字符时因编译后诊断同步重复整稿渲染导致的光标闪烁（诊断列表未变化时跳过重渲染）；图床默认支持阿里云 OSS（PUT + V1 签名 + 自定义域名），粘贴自动上传失败回退本地；插件设置面板改为“一插件一卡片”，插件菜单按插件分组加分隔。
